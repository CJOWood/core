...
class RyobiApi:
    ...
    async def get_device(self, d_id, d_url=f"{HTTP_ENDPOINT}/devices"):
        ...
        if resp:  # list not empty
            ...
            _LOGGER.error(
                "RyobiApi: Could not get information for that device. Returning nothing. "
            )
    ...
    def extract_device_info(self, response):
        ...
        for module in deviceMap:
            if module.startswith("modulePort_"):
                if any(
                    "garageDoor" in sub
                    for sub in deviceMap[module]["at"]["moduleProfiles"]["value"]
                ):
                    device_info["module_id"] = deviceMap[module]["at"]["moduleId"][
                        "value"
                    ]
                    port = device_info["port_id"] = deviceMap[module]["at"]["portId"][
                        "value"
                    ]
                    break
        ...
        device_info["garage_state"] = {
            "vacationMode": {
                ...
                "vacation_state": garage_door["vacationMode"]["enum"][
                    garage_door["vacationMode"]["value"]
                ],  # TODO Eventually define enum in const so we dont rely on server response to define enum
            },
            "sensorFlag": {
            ...
            "opMode": {
                ...
                "op_state": garage_door["opMode"]["enum"][
                    garage_door["opMode"]["value"]
                ],  # TODO Eventually define enum in const so we dont rely on server response to define enum
            },
            "doorState": {
                ...
                "state": garage_door["doorState"]["enum"][
                    garage_door["doorState"]["value"]
                ],  # TODO Eventually define enum in const so we dont rely on server response to define enum
            },
            ...
            "doorPosition": {
                ...
                "value": garage_door["doorPosition"][
                    "value"
                ],  # value of 0 - 180 -> but not?? Could be different per door?
            },
            "garageLight": {
                "lightState": {
...
class RyobiWssCtrl(RyobiApi):
        ...
        if "method" in message:
            if message["method"] == GARAGE_UPDATE_MSG:
                ...
                if message["params"]["authorized"]:
                    ...
                    _LOGGER.error(
                        "RyobiApi (WSS) Msg: Api Key? not authorized based on response form server. "
                    )
            ...
            if "result" in message["result"]:
                if message[WS_CMD_ACK]["result"] == WS_OK:
                    _LOGGER.debug(
                        "RyobiApi (WSS) Msg: Recieved OK result from server. "
                    )
    ...
    def parse_device_update(self, update):
        ...
        if self.device_id != update["varName"]:
            _LOGGER.debug(
                "RyobiApi (WSS): Recieved update for a differnt device! (%s but I am %s)",
                update["varName"],
                self.device_id,
            )
            return None
        ...
        for key in list(update.keys()):
            ...
            _LOGGER.debug(
                "RyobiApi (WSS) parse update each item in %s: %s", key, update[key]
            )
            ...
            if "garageDoor" in key:
                ...
                for item in update[key]:
                    self.garage_state["garageLight"][module_name][item] = update[key][
                        item
                    ]

            else:
                _LOGGER.error(
                    "RyobiApi (WSS) parse update: Did not recognized module: %s", key
                )
