...
class RyobiGarageDoor(CoverEntity):
    ...
    def state(self):
        ...
        try:
            ...
            _LOGGER.debug(
                "Ryobi Cover: Ryobi State (%s) mapped to HA State = %s ",
            ...
            if self.device.current_state == "Fault":
                _LOGGER.error(
                    "Roybi Cover: The Garage Door is in a 'FAULT' state. Someting is wrong!"
                )
            else:
                _LOGGER.error(
                    "Roybi Cover: Found an unsupported state, current_state: %s",
                    self.device.current_state,
                )
            return None
    ...
    def extra_state_attributes(self) -> dict:
        ...
        unixtime = (
            int(self.device.garage_state["doorState"]["lastSet"]) / 1000.0
        )  # divide by 1000 to go form ms to s
        extra_value = {
            "garage_state": self.device.current_state,
            "door_last_set": datetime.fromtimestamp(unixtime).strftime(
                "%Y-%m-%d %H:%M:%S"
            ),
            ...
            "garage_light_state": self.device.garage_state["garageLight"]["lightState"][
                "value"
            ],
            "garage_light_timer": self.device.garage_state["garageLight"]["lightTimer"][
                "value"
            ],
